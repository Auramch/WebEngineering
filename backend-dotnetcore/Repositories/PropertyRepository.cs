...
public class PropertyRepository : IRepository<Property, int>
{
    ...
    public IQueryable<Property> SimpleCollection => _context.Properties;
    public IQueryable<Property> FullCollection => _context.Properties.Include(p=> p.Location);

    public async Task<bool> CreateAsync(Property property)
    {
        // Check whether IMDb URL or title already exist
        if (await _context.Properties.AnyAsync(p => p.Url == property.Url || p.Title == property.Title))
            return false;
